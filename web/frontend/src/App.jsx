...
const StaticPlot = ({ panel, activeFile, centerFreq, windowSize, smoothing, colormap, theme, fillMode, fillColor, zmin, zmax, timeDomainMode, windowType, strideRatio }) => {
    ...
    try {
      const res = await fetch('/api/run/plot', {
        ...
        body: JSON.stringify({
          ...
          time_domain_mode: timeDomainMode,
  ...
  }, [panel.subType, activeFile, centerFreq, windowSize, smoothing, colormap, theme, fillMode, fillColor, zmin, zmax, timeDomainMode, windowType, strideRatio]);
...
function App() {
  ...
  const [timeDomainMode, setTimeDomainMode] = useState('complex');
  const [windowType, setWindowType] = useState('blackman-harris');
  const [strideRatio, setStrideRatio] = useState('0'); // 0 means auto
  ...
  const [filterParams, setFilterParams] = useState({ type: 'lowpass', cutoff1: 0, cutoff2: 0, taps: 1023 });
  const [filterOutName, setFilterOutName] = useState('');

  // Resample Modal
  const [showResampleModal, setShowResampleModal] = useState(false);
  const [resampleParams, setResampleParams] = useState({ newRate: 0, quality: 'normal' });
  const [resampleOutName, setResampleOutName] = useState('');

  // Whiten Modal
  const [showWhitenModal, setShowWhitenModal] = useState(false);
  const [whitenParams, setWhitenParams] = useState({ fftSize: 4096, alpha: 0.99, blankThreshold: 10.0, blankWindow: 1024, gain: 0.0, mode: 'compress', strength: 0.5, excessLeak: -100.0 });
  const [whitenOutName, setWhitenOutName] = useState('');
  ...
  useEffect(() => {
    ...
    setPanels(prev => {
        ...
        if (hasFFT) setTimeout(() => handleInteractiveFFT(), 0);
        if (hasPSD) setTimeout(() => handleInteractivePSD(), 0);
        if (hasTD) setTimeout(() => handleInteractiveTimeDomain(), 0);
        
        return prev;
    });
  }, [windowSize, smoothing, windowType, strideRatio, timeDomainMode]);
    ...
    try {
      const res = await fetch('/api/run/tuner', {
        ...
        body: JSON.stringify({
          ...
          quality: 'normal',
          oversample: Number(tunerOversample)
        })
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const startFilter = async () => {
    ...
    setLoading(true);
    try {
      const isComplex = fileInfo?.channels === 2;
      const c1 = parseFloat(filterParams.cutoff1) || 0;
      const c2 = parseFloat(filterParams.cutoff2) || 0;
      let mappedC1 = c1;
      let mappedC2 = c2;
      
      if (mappedC1 > mappedC2 && (filterParams.type === 'bandpass' || filterParams.type === 'bandstop')) {
        const temp = mappedC1;
        mappedC1 = mappedC2;
        mappedC2 = temp;
      }
      
      const res = await fetch('/api/run/filter', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          input_file: file,
          output_file: filterOutName,
          filter_type: filterParams.type,
          cutoff1: mappedC1 * 1e6,
          cutoff2: mappedC2 * 1e6,
          taps: Number(filterParams.taps) || 1023,
          center_freq: (isComplex && Number(centerFreq) !== 0) ? Number(centerFreq) * 1e6 : 0.0
        })
      });
      const data = await res.json();
      if (!res.ok) alert("Error filtering: " + (data.detail || JSON.stringify(data)));
      else {
        await fetchFiles();
        switchFile(filterOutName);
      }
    } catch(e) { alert("Error: " + e); }
    setLoading(false);
  };

  const startResample = async () => {
    setShowResampleModal(false);
    setLoading(true);
    try {
      const res = await fetch('/api/run/resample', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          input_file: file,
          output_file: resampleOutName,
          new_rate: resampleParams.newRate * 1e6,
          quality: resampleParams.quality
        })
      });
      const data = await res.json();
      if (!res.ok) alert("Error resampling: " + (data.detail || JSON.stringify(data)));
      else {
        await fetchFiles();
        switchFile(resampleOutName);
      }
    } catch(e) { alert("Error: " + e); }
    setLoading(false);
  };

  const startWhiten = async () => {
    setShowWhitenModal(false);
    setLoading(true);
    try {
      const res = await fetch('/api/run/whiten', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          input_file: file,
          output_file: whitenOutName,
          fft_size: whitenParams.fftSize,
          alpha: whitenParams.alpha,
          blank_threshold: whitenParams.blankThreshold,
          blank_window: whitenParams.blankWindow,
          output_gain: whitenParams.gain,
          mode: whitenParams.mode,
          strength: whitenParams.strength,
          excess_leak: whitenParams.excessLeak
        })
      });
      const data = await res.json();
      if (!res.ok) alert("Error whitening: " + (data.detail || JSON.stringify(data)));
      else {
        await fetchFiles();
        switchFile(whitenOutName);
      }
    } catch(e) { alert("Error: " + e); }
    setLoading(false);
  };
  ...
  return (
    <div className="container">
      ...
      {isSettingsModalOpen && (
        <div style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyItems: 'center', justifyContent: 'center', zIndex: 9999}}>
          <div style={{background: 'var(--panel-bg)', padding: '20px', borderRadius: '8px', minWidth: '350px', border: '1px solid var(--border-color)', boxShadow: '0 10px 25px rgba(0,0,0,0.5)'}}>
            <h3 style={{marginTop: 0, borderBottom: '1px solid var(--border-color)', paddingBottom: '10px'}}>Settings</h3>
            
            <div style={{marginBottom: '20px'}}>
              <label style={{display: 'block', marginBottom: '5px'}}>Time Domain Mode</label>
              <select value={timeDomainMode} onChange={(e) => setTimeDomainMode(e.target.value)} style={{width: '100%'}}>
                <option value="complex">I & Q (Complex)</option>
                <option value="real">Real (I)</option>
                <option value="imag">Imaginary (Q)</option>
            ...
            <div style={{marginBottom: '20px'}}>
              <label style={{display: 'block', marginBottom: '5px'}}>FFT Window Type</label>
              <select value={windowType} onChange={(e) => setWindowType(e.target.value)} style={{width: '100%'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end'}}>
              <button onClick={() => setIsSettingsModalOpen(false)} style={{background: 'var(--accent-color)', color: '#000', padding: '8px 16px', borderRadius: '4px', border: 'none', cursor: 'pointer', fontWeight: 'bold'}}>Done</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="panel" style={{flex: 1, overflowY: 'auto'}}>
          ...
          {loading && (
            <div style={{
              ...
              <div style={{
                background: 'var(--panel-bg)', padding: '15px 30px', borderRadius: '8px',
                border: '1px solid var(--accent-color)', boxShadow: '0 4px 15px rgba(0,0,0,0.5)',
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px'
              }}>
                <div style={{fontSize: '2rem', animation: 'spin 2s linear infinite'}}>⏳</div>
                <div style={{color: 'var(--accent-color)', fontWeight: 'bold'}}>Processing...</div>
              </div>
            </div>
          )}

          <div className="form-group" style={{marginTop: '15px'}}>
            <h3 style={{marginTop: 0, borderBottom: '1px solid var(--border-color)', paddingBottom: '3px', fontSize: '1.1rem'}}>DSP Processing</h3>
          ...
          <div className="form-group" style={{marginTop: '15px'}}>
            <h3 style={{marginTop: 0, borderBottom: '1px solid var(--border-color)', paddingBottom: '3px', fontSize: '1.1rem'}}>FFT Options</h3>
            <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
              ...
              <div style={{flex: 1}}>
                <label>Stride Ratio</label>
                <select value={strideRatio} onChange={(e) => setStrideRatio(e.target.value)}>
                  <option value="0">Auto (Fit to Plot)</option>
                  <option value="0.25">25% (75% Overlap)</option>
                  <option value="0.5">50% (50% Overlap)</option>
                  <option value="0.75">75% (25% Overlap)</option>
                  <option value="1.0">100% (0% Overlap)</option>
                </select>
              </div>
              <div style={{flex: 1}}>
                <label>Smoothing {displaySmoothing > 0 ? `(${displaySmoothing})` : '(Off)'}</label>
                <input type="range" min="0" max="64" value={displaySmoothing} onChange={(e) => setDisplaySmoothing(Number(e.target.value))} onPointerUp={(e) => setSmoothing(Number(e.target.value))} onKeyUp={(e) => setSmoothing(Number(e.target.value))} style={{width: '100%', accentColor: 'var(--accent-color)'}} />
              </div>
            </div>
          </div>
              ...
              {panel.type === 'interactive' ? (
                  ...
                  <div style={{flex: 1, position: 'relative', minHeight: '200px', overflow: 'hidden'}}>
                    <StaticPlot 
                      ...
                      timeDomainMode={timeDomainMode}
                      windowType={windowType}
                      strideRatio={strideRatio}
                    />
                  </div>
              )}
            </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
              ...
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Oversample Factor</label>
                <select 
                  value={tunerOversample} 
                  onChange={e => setTunerOversample(Number(e.target.value))}
                  style={{width: '100%', padding: '8px', boxSizing: 'border-box'}}
                >
      ...
      {showFilterModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{marginTop: 0, color: 'var(--accent-color)'}}>FIR Filter</h2>
            <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Filter Type</label>
                <select value={filterParams.type} onChange={e => setFilterParams({...filterParams, type: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}}>
              ...
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Cutoff 1 (MHz)</label>
                <input type="text" value={filterParams.cutoff1} onChange={e => setFilterParams({...filterParams, cutoff1: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              {(filterParams.type === 'bandpass' || filterParams.type === 'bandstop') && (
                <div>
                  <label style={{display: 'block', marginBottom: '5px'}}>Cutoff 2 (MHz)</label>
                  <input type="text" value={filterParams.cutoff2} onChange={e => setFilterParams({...filterParams, cutoff2: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
                </div>
              )}
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Taps</label>
                <input type="text" value={filterParams.taps} onChange={e => setFilterParams({...filterParams, taps: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Output Filename</label>
                <input type="text" value={filterOutName} onChange={e => setFilterOutName(e.target.value)} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
                <button onClick={() => setShowFilterModal(false)} style={{flex: 1, background: 'var(--border-color)', margin: 0}}>Cancel</button>
                <button onClick={startFilter} style={{flex: 1, background: 'var(--accent-color)', color: '#000', margin: 0, fontWeight: 'bold'}}>Filter</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showResampleModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{marginTop: 0, color: 'var(--accent-color)'}}>Resample</h2>
            <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>New Rate (MHz)</label>
                <input type="text" step="any" value={resampleParams.newRate} onChange={e => setResampleParams({...resampleParams, newRate: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Quality</label>
                <select value={resampleParams.quality} onChange={e => setResampleParams({...resampleParams, quality: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}}>
              ...
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Output Filename</label>
                <input type="text" value={resampleOutName} onChange={e => setResampleOutName(e.target.value)} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
                <button onClick={() => setShowResampleModal(false)} style={{flex: 1, background: 'var(--border-color)', margin: 0}}>Cancel</button>
                <button onClick={startResample} style={{flex: 1, background: 'var(--accent-color)', color: '#000', margin: 0, fontWeight: 'bold'}}>Resample</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showWhitenModal && (
        <div style={{
          ...
          <div style={{
            background: 'var(--panel-bg)', padding: '20px', borderRadius: '8px',
            width: '400px', border: '1px solid var(--border-color)', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', margin: '20px'
          }}>
            <h2 style={{marginTop: 0, color: 'var(--accent-color)'}}>Whiten / Pulse Blanker</h2>
            <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>FFT Size</label>
                <input type="text" value={whitenParams.fftSize} onChange={e => setWhitenParams({...whitenParams, fftSize: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Alpha (Smoothing)</label>
                <input type="text" step="any" value={whitenParams.alpha} onChange={e => setWhitenParams({...whitenParams, alpha: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Blanking Threshold (x Avg Power)</label>
                <input type="text" step="any" value={whitenParams.blankThreshold} onChange={e => setWhitenParams({...whitenParams, blankThreshold: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Mode</label>
                <select value={whitenParams.mode} onChange={e => setWhitenParams({...whitenParams, mode: e.target.value})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}}>
                  <option value="compress">Spectral Compression</option>
                  <option value="griffiths">Griffiths Algorithm</option>
                </select>
              </div>
              {whitenParams.mode === 'compress' && (
                <div>
                  <label style={{display: 'block', marginBottom: '5px'}}>Strength</label>
                  <input type="text" step="any" value={whitenParams.strength} onChange={e => setWhitenParams({...whitenParams, strength: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
                </div>
              )}
              {whitenParams.mode === 'griffiths' && (
                <div>
                  <label style={{display: 'block', marginBottom: '5px'}}>Excess Leak (dB)</label>
                  <input type="text" step="any" value={whitenParams.excessLeak} onChange={e => setWhitenParams({...whitenParams, excessLeak: Number(e.target.value)})} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
                </div>
              )}
              <div>
                <label style={{display: 'block', marginBottom: '5px'}}>Output Filename</label>
                <input type="text" value={whitenOutName} onChange={e => setWhitenOutName(e.target.value)} style={{width: '100%', padding: '8px', boxSizing: 'border-box'}} />
              </div>
              <div style={{display: 'flex', gap: '10px', marginTop: '10px'}}>
                <button onClick={() => setShowWhitenModal(false)} style={{flex: 1, background: 'var(--border-color)', margin: 0}}>Cancel</button>
                <button onClick={startWhiten} style={{flex: 1, background: 'var(--accent-color)', color: '#000', margin: 0, fontWeight: 'bold'}}>Whiten</button>
              </div>
            </div>
          </div>
        </div>
